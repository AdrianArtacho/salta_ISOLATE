...
while cap.isOpened():
    ...
    if roi_defined and ix != -1 and iy != -1 and ex != -1 and ey != -1:
        ...
        # Draw rectangle with rounded corners only for editing, not in the output
        if drawing:
            cv2.rectangle(display_frame, (ix, iy), (ex, ey), (0, 255, 0), 2, cv2.LINE_AA)
    ...
    if roi_defined:
        ...
        cv2.rectangle(output_mask, (ix, iy), (ex, ey), 255, -1)
        output_blurred_frame = cv2.GaussianBlur(output_frame, (51, 51), 0)
        output_outside_roi = cv2.bitwise_and(output_blurred_frame, output_blurred_frame, mask=cv2.bitwise_not(output_mask))
        output_frame = cv2.add(cv2.bitwise_and(output_frame, output_frame, mask=output_mask), output_outside_roi)
    else:
        output_frame = frame_buffer[-1].copy()

...
def draw_rectangle(event, x, y, flags, param):
    ...
    if paused:
        ...
        # When the left mouse button is released, finalize the rectangle
            ...
            # Draw the rectangle permanently on the frame (with green outline)
            cv2.rectangle(frame, (ix, iy), (x, y), (0, 255, 0), 3)  # Green outline, thickness 3
...
def play_video_and_capture_rectangles(video_path, output_file):
    ...
    while video.isOpened():
        ...
        # Re-draw all rectangles that have been drawn so far
        for ix, iy, x, y, _ in rectangles:
            cv2.rectangle(frame, (ix, iy), (x, y), (0, 255, 0), 3)  # Green outline, thickness 3

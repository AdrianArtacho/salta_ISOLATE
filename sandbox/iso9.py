...
def rewind_frames(steps):
    ...
    if frame_number > steps and len(frame_buffer) > steps:
...
# Function to apply gradient mask
def apply_gradient_mask(frame, mask):
    # Create a gradient mask
    gradient_mask = np.zeros_like(frame, dtype=np.uint8)
    ellipse_mask = np.zeros_like(frame, dtype=np.uint8)
    cv2.ellipse(ellipse_mask, ((ix + ex) // 2, (iy + ey) // 2), ((ex - ix) // 2, (ey - iy) // 2), 0, 0, 360, (255, 255, 255), -1)
    gradient_mask = cv2.GaussianBlur(ellipse_mask, (51, 51), 0)

    # Apply gradient mask to the frame
    # Adjust the weighting here for more transparency within the ROI
    frame = cv2.addWeighted(frame, 1.0, gradient_mask, 0.3, 0)
    return frame
...
while cap.isOpened():
    ...
    if roi_defined and ix != -1 and iy != -1 and ex != -1 and ey != -1:
        ...
        cv2.ellipse(mask, ((ix + ex) // 2, (iy + ey) // 2), ((ex - ix) // 2, (ey - iy) // 2), 0, 0, 360, 255, -1)
        ...
        # Apply gradient mask for smoother transition
        display_frame = apply_gradient_mask(display_frame, mask)

        # Draw rectangle only for editing, not in the output
        if drawing:
            cv2.ellipse(display_frame, ((ix + ex) // 2, (iy + ey) // 2), ((ex - ix) // 2, (ey - iy) // 2), 0, 0, 360, (0, 255, 0), 2)  # Ellipse for ROI

    cv2.imshow('Frame', display_frame)

    # Write to output without the ROI ellipse
    if roi_defined:
        output_frame = apply_gradient_mask(frame_buffer[-1].copy(), mask)
    else:
